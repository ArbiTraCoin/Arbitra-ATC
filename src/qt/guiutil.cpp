...
{
    // Convert arbitra:// to arbitra:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("arbitra://"))
    {
        uri.replace(0, 12, "arbitra:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Arbitra.lnk";
}
...
{
    return GetAutostartDir() / "arbitra.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Arbitra-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  arbitra-qt [" + tr("command-line options") + "]                     " + "\n";
